...
def train_and_compare():
    ...
    if missing_columns:
        print(f"❌ Dataset thiếu cột: {', '.join(missing_columns)}")
        return

    df = df.dropna(subset=required_columns)
    if df.empty or df["label"].nunique() < 2:
        print("❌ Dataset cần dữ liệu hợp lệ của ít nhất 2 nhãn để huấn luyện.")
        return
    ...
    if min_class_count < 2:
        print("❌ Mỗi nhãn cần ít nhất 2 mẫu để chia dữ liệu huấn luyện.")
        return

    test_size = max(0.2, y.nunique() / len(y))
